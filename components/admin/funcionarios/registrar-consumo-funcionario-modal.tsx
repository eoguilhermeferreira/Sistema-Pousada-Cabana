...
import { registrarConsumoFuncionario } from "@/services/funcionario-consumo-service";
...
interface RegistrarConsumoFuncionarioModalProps {
  open: boolean;
...
export function RegistrarConsumoFuncionarioModal({
  open,
  onOpenChange,
  funcionarioId,
  onRegistrado,
}: RegistrarConsumoFuncionarioModalProps) {
  ...
  async function handleRegistrar() {
    ...
    setError("");
    try {
      await registrarConsumoFuncionario(funcionarioId, selecionado.id, quantidade);
      onRegistrado();
      onOpenChange(false);
    } catch (err) {
      setError(
  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange}>
      <ModalContent title="Registrar consumo" className="max-w-lg">
              ...
              <label className="flex flex-col gap-1.5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setQuantidade((q) => Math.min(selecionado.quantidade, q + 1))
                    }
                    className="flex size-10 items-center justify-center rounded-xl border border-gray-text/20 text-gray-text hover:border-gray-text/40"
                  >
